...
Locale files live under `src/assets/i18n/`. Spanish is `es.json` (the source of truth). The supported set mirrors `AGENTS.md` § Technologies and the Transloco config in `src/app/app.config.ts` (`availableLangs: ['es', 'ca', 'en']`):
...
The roadmap tracks **`ca-ES-valencia`** (Valencian) and **`en-GB`** as the first-party go-live locales (`AGENTS.md` § Roadmap, Multilingual phase). Today they are served from `ca.json` / `en.json`; a future regional split is added by **extending this table and `TARGET_LANGS` in `scripts/i18n-sync.mjs` together — never silently**.
